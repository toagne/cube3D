...
/*   Updated: 2024/11/14 16:43:56 by mpellegr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_keyboard(mlx_key_data_t keydata, void *param)
{
	...
	if (table->player_angle > 0 && table->player_angle < 180)
		mpyc = (table->player_y + avoid_wall_collision) / T_SIZE;
	else
		mpyc = (table->player_y - avoid_wall_collision) / T_SIZE;
	if (table->player_angle > 270 || table->player_angle < 90)
		mpxc = (table->player_x + avoid_wall_collision) / T_SIZE;
	else
		mpxc = (table->player_x - avoid_wall_collision) / T_SIZE;
	printf("mpy = %d\n", mpy);
	printf("mpx = %d\n", mpx);
	...
	{
		if (table->map[mpy * (table->columns + 1) + mpxc] != '1')
			table->player_x+=table->player_delta_x * 5;
		if (table->map[mpyc * (table->columns + 1) + mpx] != '1')
